...
const ResItem = ({ data, saveToLocal, removeFromLocal }) => {
	let main_photo =
		'https://www.texastechnologies.com/wp-content/uploads/2016/10/product_i_m_img_not_available_1_1_1_1_1_1_1_1_1_1_1_1_1_1_1_1_1_1.png';
	...
	if (data.build && data.build.year) {
		detail_list.push(
			<span key="details-year" name="detail-elem">
				{data.build.year}
			</span>
		);
		detail_list.push(
			<span key="point1" name="detailPoint">
				•
			</span>
		);
	}
	if (data.miles) {
		detail_list.push(
			<span key="details-miles" name="detail-elem">
				{data.miles.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,').replace('.00', '')} miles
			</span>
		);
		detail_list.push(
			<span key="point2" name="detailPoint">
				•
			</span>
		);
	}
	if (data.build && data.build.fuel_type) {
		detail_list.push(
			<span key="details-fuel" name="detail-elem">
				{data.build.fuel_type.replace('Unleaded', '')}
			</span>
		);
		detail_list.push(
			<span key="point3" name="detailPoint">
				•
			</span>
		);
	}
		...
		<div className="ResItem container-fluid position-relative mb-4 bg-white shadow-sm p-0">
			...
			<div className="row">
				<div className="col-4 col-lg-5">
					<img src={main_photo} className="img-fluid" alt="car listing" />
				</div>
					...
					<h5 className="d-flex justify-content-between position-relative m-0 py-1 py-lg-3 border-bottom">
						...
						<span className="ResItem__save-icon position-absolute">
							{isSaved ? (
								...
								<i className="fas fa-heart text-info" onClick={() => saveToLocal(data)} />
							)}
						</span>
					</h5>
				...
				<Link
					to={`/listing/${data.id}`}
					className="ResItem__view-btn d-none d-sm-block position-absolute btn btn-sm btn-info"
				>
					View
				</Link>
				<Link
					to={`/listing/${data.id}`}
					className="ResItem__view-link d-sm-none position-absolute text-info border-bottom border-info"
				>
					View
				</Link>
			</div>
