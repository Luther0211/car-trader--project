...
const ResItem = ({ data }) => {
    ...
    if(data.build && data.build.fuel_type) {
        detail_list.push(<span key='details-fuel'>{data.build.fuel_type}</span>)
        detail_list.push(<span key='point3' name='detailPoint'>•</span>)
    }
    if(data.build && data.build.transmission) {
        detail_list.push(<span key='details-trans'>{data.build.transmission}</span>)
    }
    ...
    return (
        <div className='ResItem container-fluid mb-4 bg-white shadow-sm p-0'>
            ...
            <div className='row'>
                ...
                <div className='col-8 col-lg-7 pl-0 pr-4 position-relative'>
                    <h5 className="d-flex justify-content-between m-0 my-lg-3 pb-lg-3 border-bottom">
                    ...
                    <div className='ResItem__details p-2 mt-4 d-flex justify-content-between'>
                        { detail_list }
                    </div>
                    
                    <button className='ResItem__view-btn position-absolute mr-4 btn btn-info'>View</button>
